...
# ---------------------------------------------------------------------------
# AI 研究助手 / 因子假设持久化(issue #84)
#
# 红线:
# * 这些表只服务研究与教育,不写入实盘 orders/fills/positions/audit_logs;
# * 失败 / 被拒绝的假设与草案同样保留,不得丢失;
# * validated_oos 只能由绑定的 #57 research_experiments 机器结果决定。
# ---------------------------------------------------------------------------


class FactorHypothesisModel(Base, IdMixin):
    """结构化因子假设持久化(issue #65 / #84)。

    生命周期:proposed → approved_for_research → in_sample →
    validated_oos / rejected;任意 → superseded。
    失败 / 被拒绝记录同样保留用于审计。
    """

    __tablename__ = "factor_hypotheses"
    ...
    input_fields: Mapped[list[str]] = mapped_column(JSON)
    decision_timing: Mapped[str] = mapped_column(String(16))
    formula: Mapped[str] = mapped_column(Text)
    direction: Mapped[str] = mapped_column(String(16))
    applicable_assets: Mapped[list[str]] = mapped_column(JSON, default=list)
    expected_failure_scenarios: Mapped[list[str]] = mapped_column(JSON, default=list)
    parameters: Mapped[list[dict[str, object]]] = mapped_column(JSON, default=list)
    references: Mapped[list[dict[str, object]]] = mapped_column(JSON, default=list)
...
class FactorHypothesisExperimentModel(Base, IdMixin):
    """因子假设实验登记持久化(issue #84)。

    绑定模型 / 提示词 / 数据 / 代码版本与 #57 机器验证实验,
    用于审计与可复现。
    """

    __tablename__ = "factor_hypothesis_experiments"

    experiment_id: Mapped[str] = mapped_column(String(32), unique=True, index=True)
    hypothesis_id: Mapped[str] = mapped_column(
        String(32),
        ForeignKey("factor_hypotheses.hypothesis_id", ondelete="RESTRICT"),
        index=True,
    )
...
class AIDraftModel(Base, IdMixin):
    """AI 草案 / 回答持久化(issue #84)。

    所有 AI 建议必须以草案或假设形式持久化,显示来源、模型/提示词版本、
    不确定性和审批状态。失败 / 被拒绝草案同样保留。
    """

    __tablename__ = "ai_drafts"

    draft_id: Mapped[str] = mapped_column(String(48), unique=True, index=True)
    kind: Mapped[str] = mapped_column(String(32), index=True)
    provenance: Mapped[dict[str, object]] = mapped_column(JSON)
    status: Mapped[str] = mapped_column(String(16), index=True)
    payload: Mapped[dict[str, object]] = mapped_column(JSON)
    uncertainty: Mapped[str] = mapped_column(String(16), default="medium")
    references: Mapped[list[dict[str, object]]] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), index=True
    )
    ...
    rejection_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    consumed_ref: Mapped[str | None] = mapped_column(String(64), nullable=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
...
class AIAuditEventModel(Base, IdMixin):
    """AI / 假设审批审计事件持久化(issue #84)。

    只追加,不修改,不删除。用于重启恢复与审计追踪。
    不写入实盘 ``audit_logs`` 表。
    """
    ...
    draft_id: Mapped[str | None] = mapped_column(String(48), nullable=True, index=True)
    actor: Mapped[str] = mapped_column(String(128))
