...
def register(mcp: MCPServer) -> None:
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "换窗复测承担,负 IC 方向型因子过门不构成缺陷。相关性证据 fail-closed"
            "(#311):screen 证据缺 correlation 或空对象报 screen.correlation_"
            "missing,不因空 mapping 静默通过;correlation_baselines=[](横截面"
            "确无 baseline 因子可比)报 screen.correlation_not_evaluated 并附"
            "修复路径(screen run 横截面需含至少一个非用户因子特征)。失败返回"
