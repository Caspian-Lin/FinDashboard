...
特征快照属于研究域计算,任务状态只保存在当前进程内(API 或 MCP server)。
管理器不持有数据库 session,后台 runner 必须自行创建独立 session,避免把
请求 session 跨越请求生命周期传入任务。

该模块是纯标准库实现(asyncio/dataclasses/datetime),无项目依赖,放在
``finboard-backtest`` 以便 API 路由与 MCP server 共享(两者都依赖本包)。
"""
