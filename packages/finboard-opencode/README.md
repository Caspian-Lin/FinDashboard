...
| `access.py` | `AccessCredentialIssuer` —— 签发访问凭证(网关启用即签发,#121 重构后无 conversation 绑定) |
...
- **签发** 访问凭证(网关启用即签发 Web URL + basic auth,#121 重构后不绑定 conversation);
- **不暴露** 未授权端口:OpenCode 由 basic auth 保护。
...
2. **环境变量严格白名单**:`-e` 只注入 LLM key / MCP token / basic auth 凭证;
3. **网络隔离**:强制 `127.0.0.1` 绑定 + basic auth,不暴露公网;
4. **会话数据持久化**:命名卷 `opencode-data` → `/root/.local/share/opencode`(会话 DB),
   `opencode-config` → `/root/.config/opencode`(auth),容器重建不丢数据;
...
| `POST` | `/api/opencode/access` | 签发访问凭证(网关启用即签发) | 503 |

`POST /access`(#121 重构后不绑定 conversation):网关启用即返回
`{web_url, username, password, agent_name}`。前端 iframe 用 basic auth URL 嵌入:
