...
export default function Fills() {
  ...
  return (
    <div>
      ...
      {fills.length === 0 ? (
        <div className="text-gray-400 text-center py-12">无成交记录</div>
      ) : (
          ...
          <tbody>
            {fills.map((f) => (
              <tr key={f.fill_id} className="border-t">
                <td className="px-4 py-2 font-mono text-xs text-gray-500">{f.fill_id}</td>
                ...
                <td className="px-4 py-2 text-right text-gray-500">{f.commission}</td>
                <td className="px-4 py-2 text-gray-500">{new Date(f.filled_at).toLocaleTimeString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
