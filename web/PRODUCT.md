...
控制台页（`/control`）是实盘交易的高风险操作入口。Kill Switch 由交易内核执行，
页面只提交指令、不绕过内核；所有档位切换均需在确认弹窗中二次确认。

### 五档 Kill Switch

| 档位 | 含义 | 影响范围 |
|------|------|----------|
| 恢复正常（off） | 解除中止状态，恢复新单提交 | 立即恢复交易行为，切换前请确认风险已解除 |
| 暂停新单（no_new_orders） | 禁止提交新订单 | 已提交订单不受影响，仍可撤单、查询与减仓 |
| 仅减仓（reduce_only） | 禁止开新仓 | 仅允许减少现有持仓的卖出操作 |
| 撤全部（cancel_all） | 撤销全部活动订单 | 由内核向券商发撤单（不可撤销的订单除外），随后进入仅减仓约束；不可自动恢复 |
| 全局停止（halt） | 停止一切交易行为 | 禁止新单、撤单与减仓，策略暂停；不可自动恢复，需人工逐级解除 |

### 确认流程

1. 点击任一档位按钮只打开确认弹窗，不发送任何请求。
2. 弹窗展示当前状态、目标状态、影响范围、不可逆/恢复提示与操作原因输入框。
3. 取消、关闭（X）或按 Escape 均不产生状态变更请求；确认后只发送一次请求。
4. 请求进行中（pending）所有档位按钮与弹窗确认按钮均禁用，弹窗不可关闭，
   避免重复提交；成功/失败通过 `role=status` / `role=alert` 区域播报。
5. 失败或后端拒绝时弹窗保留并显示原因，可显式重试（再次点击确认），不会静默重发。

原因字段沿用后端 `reason`（写入审计日志），不新增前端校验，不绕过后端权限。
...
4. **实盘二次确认**：下单、撤单和 Kill Switch 切换操作均需 Dialog 二次确认
