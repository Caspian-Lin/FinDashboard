...
> 本文档是 FinDashboard 的**内部开发指南**(原仓库主 README,2026-09-04 随公开
> 改版迁入 docs/),包含完整的本地部署、测试隔离、Worker 运维与研究平台细节。
> 面向外部读者的项目总览见根目录 [README.md](../README.md)。
...
(Ctrl-C 先向 Worker 发优雅停止信号、按宽限等 in-flight 收尾后强杀兜底,
issue #307)。
...
uv run finboard worker run --shutdown-grace 30
                                          # 停机宽限 30s:停止信号后先等 in-flight 跑完(issue #307)
...
时父进程先向各子进程组发优雅停止信号、按 settings 宽限等子进程自行收敛后
强杀兜底(宽限对齐 `FINBOARD_WORKER_SHUTDOWN_GRACE_SECONDS`,默认 0 即立即
terminate,主动停止退出码 0,issue #307);子进程崩溃不自动重启 —— 未完成
任务由 lease 过期回收后重排,重新执行命令即可。回滚 = `--workers 1`(默认)
+ revert。

**优雅停机与退出码(issue #307)**:收到停止信号后 worker 停止领取新任务并
收尾 in-flight —— 宽限 `FINBOARD_WORKER_SHUTDOWN_GRACE_SECONDS`(默认 0 =
立即取消 in-flight,即时停机)内未完成任务继续跑完、超时才取消;等待中第二
次 Ctrl-C 立即强退。**退出码约定:优雅完成 0(含宽限超时后的取消兜底),
强退/异常非 0(强退 = 130)**。被取消 / 硬杀的未完成任务不丢:统一由 lease
过期回收 → `interrupted` → 退避重排(attempt 上限内)。`finboard dev` 停机
同样先优雅后强杀:向 worker 子进程组发 CTRL_BREAK(Windows)/ SIGTERM(POSIX)
并给同一宽限(settings 为 0 时给 10s 兜底窗口),超时才 `taskkill`/kill。
`--shutdown-grace <秒>` 可临时覆盖宽限,多进程形态下同步转发子进程。
...
API、迁移和三个示例见 [无代码研究策略规格](research_strategy_spec.md)。
...
[统一研究回测生命周期](research_run_lifecycle.md)。
...
[持久化模拟交易环境](simulation_trading.md)。
