...
| 策略规格 / 预设(registry/template/validate/draft/publish/rollback/diff/preset CRUD;feature_graph 可按 `u_<name>` 引用沙箱用户因子,仅 active+promotion_status=passed 可引用,#217/#219) | `finboard.strategy.*` / `.preset.*`(16 个) | ✅ #126 | 8 只读 + 8 自主执行 |
