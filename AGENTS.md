...
- **MCP 回测/入队工具易用性（#190）**：`selection.factor_version` 校验报错列出合法枚举 `[v1]`，且与特征快照 `framework_version`（`FACTOR_LAB_SCHEMA_VERSION`=v2）是**两个独立命名空间**（前者=选股规则目录版本，后者=快照 schema 版本），工具描述/文档已标注勿混传；`finboard_backtest_grid_get` 默认 `equity_mode=none` 不返回 equity 曲线（只留 `equity_point_count` 点数提示，响应最轻），显式 `summary`/`full` 才返回（此前 9 组合 × 200 点约 100-200KB 易被 MCP 客户端截断）；`finboard_run_queue` 描述附完整 payload 模板与各必填字段取值来源，澄清 `code_version` 为 **run 自身冻结字段**（冻结进 manifest/checksum 供追溯，与数据集发布的 `code_version` 同名但互不校验）。仅改 MCP 工具描述/默认返回体与文档，不触及回测语义与数据落库。纯离线研究域，不连 broker 不下单。
