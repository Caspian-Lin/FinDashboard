...
- **OpenCode agent 模型解钉与 provider 模型同步（#242）**：`finboard-researcher` 不再钉死模型——移除 `.opencode/agent/finboard-researcher.md` frontmatter 的 `model` 与 `.opencode/opencode.json` 中被 `.md` 覆盖的死配置 `agent` 块（**agent 唯一权威 = `.opencode/agent/*.md`**，默认模型跟随 OpenCode Web UI / 会话选择）。容器启动渲染 runtime `opencode.json` 时，对声明 `options.baseURL` 的 config provider（如 deepseek）经 OpenAI 兼容 `GET /models` 拉取模型 id **只增不改**合并（`{env:VAR}` 解析不到则跳过该 provider；结果缓存 `.opencode/runtime/provider-model-cache.json`，拉取失败回退缓存）。接入面（baseURL/headers/鉴权）永以仓库文件为准，远端响应只贡献 `models` id；渲染在宿主机侧完成、容器 `:ro` 挂载，agent 不可写。settings `opencode_model_sync_enabled`（默认开；false 即回滚，渲染期零网络请求）。配置存储事实：auth 连接的 provider 持久化在 data 卷 `auth.json`（UI 徽标「API 密钥」），config 声明的 provider 徽标「配置」，同 id 与 models.dev 目录合并。纯研究运行时配置，不连 broker 不下单。
