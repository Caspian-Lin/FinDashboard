...
- **OpenCode Web 研究工作台前端整合边界（#111）**：前端 `ResearchWorkbench` 页面（`/research/workbench`）是 OpenCode Web 与 FinBoard 研究页面的产品整合入口，分为 Part 1（FinBoard 控制面：会话列表 / 状态 / 关键事件摘要 / 审批产物关联）与 Part 2（OpenCode Web 交互面：iframe 跨源嵌入，复用 #118 的 `web_url` + basic auth 凭证）。**OpenCode Web 是研究交互层，FinBoard API/MCP 是事实来源与权限/审批边界**：前端 iframe 只能打开 FinBoard 授权绑定的 ACTIVE `conversation_id`（凭证由 `/api/opencode/access` 对 ACTIVE 会话签发，前端不缓存、不展示明文凭证、不手动输入任意 `session_id`）；写操作（创建 ResearchRun / 回测 / 模拟盘）仍经 FinBoard MCP 草案 + 人工审批，前端不绕过；关键事件摘要只展示 #109 `KEY_EVENT_TYPES` 投影（message/tool/error/session 等），token 级增量不落库、不展示，完整历史由 OpenCode Web 提供。`opencode_web_enabled=false`（网关 503）时前端降级到现有 `AIResearch`（`/research/ai`），保留为兼容入口。前端不连接实盘 broker/账户/订单/持仓/Kill Switch；纯前端改动，删除新增页面与路由即可回滚。
