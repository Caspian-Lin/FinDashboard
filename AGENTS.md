...
- **研究代码验证门与晋级链路（#219）**：MCP `finboard_research_code_submit` 只登记 `draft/promotion_status=pending`，不会把未验证代码放入正式组合或模拟盘白名单；`finboard_research_code_promote` 必须同时绑定同一 artifact/name/kind/commit 的 screen 运行与 #57 `validated_oos + final_test_unsealed=true` 实验，并通过 IC、换手率、相关性上限后才转为 `active/passed`。失败证据保留在 draft，旧版本回滚也只重新建立 draft，retired/未晋级引用必须 fail-visible。
  ```text
